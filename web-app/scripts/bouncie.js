...
// A function that will update the position of the ball is also needed. Lets create one
    function update() {
        ...
        var amountToRemove = ball.addMore / 5;
        ball.y += (ball.vy - amountToRemove);
        if (ball.addMore <= 1) {
            ball.addMore = 0
        }
        ball.addMore -= amountToRemove;
    ...
    return function (color) {
        var BouncieViewModel = this

        BouncieViewModel.color = ko.observable(color)
        BouncieViewModel.y = ko.observable(50)

        setTimeout(function () {
            BouncieViewModel.color('red')
        }, 2000)
...
// 4) the method to draw or paint it on the canvas
        ...
        ball = {
            ...
            draw: function () {
                ...
                ctx.fillStyle = BouncieViewModel.color();
                ctx.fill();
                ctx.closePath();
            }
        };
